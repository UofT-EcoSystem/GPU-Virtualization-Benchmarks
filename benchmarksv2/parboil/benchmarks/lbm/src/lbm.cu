...
void CUDA_LBM_initializeGrid( float** d_grid, float** h_grid, cudaStream_t & stream ) {
	const size_t size   = TOTAL_PADDED_CELLS*N_CELL_ENTRIES*sizeof( float ) + 2*TOTAL_MARGIN*sizeof( float );

  printf("LBM --- dst: %p, src: %p, size: %lu\n", *d_grid - REAL_MARGIN, *h_grid - REAL_MARGIN, size);

	cudaMemcpyAsync(*d_grid - REAL_MARGIN, *h_grid - REAL_MARGIN, size, cudaMemcpyHostToDevice, stream);
