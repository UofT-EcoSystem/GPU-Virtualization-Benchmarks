...
{	
	...
	*alloc_t += (double) tmp_t;
	...
	if(isCoordChanged || iter == 0)
	{
		CUDA_SAFE_CALL( cudaMemcpyAsync(coord_d,  coord_h,	 num * dim * sizeof(float), cudaMemcpyHostToDevice, streamcluster::gpusim_stream) );
	}
	CUDA_SAFE_CALL( cudaMemcpyAsync(center_table_d,  center_table,  num * sizeof(int),   cudaMemcpyHostToDevice, streamcluster::gpusim_stream) );
	CUDA_SAFE_CALL( cudaMemcpyAsync(p,  points->p,				   num * sizeof(Point), cudaMemcpyHostToDevice, streamcluster::gpusim_stream) );
	
	CUDA_SAFE_CALL( cudaMemsetAsync((void*) switch_membership_d, 0,			num * sizeof(bool), streamcluster::gpusim_stream)  );
	CUDA_SAFE_CALL( cudaMemsetAsync((void*) work_mem_d,  		0, stride * (nThread + 1) * sizeof(float), streamcluster::gpusim_stream) );
  ...
  while (!can_exit) {
	...
	*kernel_t += (double) tmp_t;
	...
	//=======================================
	CUDA_SAFE_CALL( cudaMemcpyAsync(work_mem_h, 		  work_mem_d, 	stride * (nThread + 1) * sizeof(float), cudaMemcpyDeviceToHost, streamcluster::gpusim_stream) );
	CUDA_SAFE_CALL( cudaMemcpyAsync(switch_membership, switch_membership_d,	 num * sizeof(bool),  cudaMemcpyDeviceToHost, streamcluster::gpusim_stream) );
