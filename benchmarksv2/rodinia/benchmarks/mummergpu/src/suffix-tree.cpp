...
SuffixNode::SuffixNode(int s, int e, int leafid,
...
void SuffixNode::setPrintParent(int min_match_len)
{
  if (!m_parent)
  {
    // at root
...
const char * SuffixNode::str(const char * refstr)
{
  return substr(refstr, m_start, m_end-m_start+1);
}

int SuffixNode::len(int i/*=-1*/)
{
  if (i != -1)
  {
    if (i < m_end)
...
ostream & SuffixNode::printLabel(ostream & os, const char * refstr)
{
...
ostream & SuffixNode::printNodeLabel(ostream & os)
{
  os << m_nodeid;
  return os;
}

ostream & SuffixNode::printEdgeLabel(ostream & os, const char * refstr)
{
...
void SuffixTree::printNodeDot(SuffixNode * node, ostream & dfile)
{
  int children = 0;
  for (int i = 0; i < basecount; i++)
  {
...
void SuffixTree::printDot(const char * dotfilename)
{
  ofstream dfile;
...
void SuffixTree::printNodeText(ostream & out, SuffixNode * n, int depth)
{
...
void SuffixTree::printText(ostream & out)
{
...
void SuffixTree::printTreeSorted(ostream & out, SuffixNode * node, const string & pathstring)
{
  bool isLeaf = true;
...
void SuffixTree::printTreeFlat(ostream & out)
{
...
void SuffixTree::printNodeFlat(ostream & out, SuffixNode * node)
{
  out << node->id()           << "\t"
      << node->m_parent->id() << "\t"
      << node->m_suffix->id() << "\t"
...
#if VERIFY
void SuffixTree::setNodePath(SuffixNode * node, const string & parentString)
{
...
int SuffixTree::verifyNodeSuffixLinks(SuffixNode * node, int & linkcount)
{
  int errs = 0;
...
void SuffixTree::verifySuffixLinks()
{
  cerr << endl;
