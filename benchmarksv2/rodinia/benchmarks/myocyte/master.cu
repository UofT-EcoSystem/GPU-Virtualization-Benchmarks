...
void master(fp timeinst,
  ...
  set_and_check(myocyte::uid, true);
  while (!set_and_check(myocyte::uid, true)) {
    usleep(100);
  }
  ...
  if (can_exit) {
	...
	cudaMemcpyAsync(d_initvalu, initvalu, d_initvalu_mem, cudaMemcpyHostToDevice, myocyte::stream);
	cudaMemcpyAsync(d_params, parameter, d_params_mem, cudaMemcpyHostToDevice, myocyte::stream);

	threads.x = NUMBER_THREADS;
	threads.y = 1;
	blocks.x = 2;
	blocks.y = 1;
	kernel<<<blocks, threads, 0, myocyte::stream>>>(	timeinst,
															d_initvalu,
															d_finavalu,
															d_params,
															d_com);
