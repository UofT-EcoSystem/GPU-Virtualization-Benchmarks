...
def parse_csv(directory, graph_type):
    ...
    for t in types:
        ...
        def skip_call(x):
            if t == 'time':
                return x in range(n_skip) or x == (n_skip + 1)
            else:
                return x in range(n_skip)

        data = pd.read_csv(full_filename, delimiter=',', skiprows=skip_call,
                           thousands=',')
        results[t] = data
...
def time(time_df):
    ...
    # total = time_df.iloc[-1]['Start'] +
    # time_df.iloc[-1]['Duration'] - time_df.iloc[0]['Start']
    ...
    result.rename(columns={'Name': 'Kernel Name', 'Duration': 'Importance'},
                  inplace=True)
...
def join(kmap, df_time, df, drop_threshold, util=False):
    ...
    for index, row in df.iterrows():
        ...
        if matched.size == 0:
            ...
            if matched.shape[0] != 1:
                print('{} has {} matches'.format(row['Kernel Name'],
                                                 matched.shape[0]))
    ...
    xticks = ['{}\n{:.2f}%'.format(i, result.iloc[i]['Importance']) for i in
...
def instmix(args, df_time, df, kmap):
    ...
    k_metrics = df.groupby(['Kernel Name', 'Metric Name'], as_index=False)[
        ['Metric Value']].sum()
    ...
    for metric in metrics:
        metric_col = k_metrics.loc[k_metrics['Metric Name'] == metric][
            'Metric Value'].array
...
def mem(args, df_time, df, kmap):
    ...
    trans_df['REGISTER_USAGE'] = trans_df['REGISTER_USAGE'] * 1024 * trans_df[
        'OCCUPANCY'] / (64 * 1024)
    blocks_per_sm = trans_df['OCCUPANCY'] * 32 * 32 / trans_df['Block Size']
    trans_df['SHARED_USAGE'] = (trans_df['DYNAMIC_SHARED_USAGE']
                                + trans_df['STATIC_SHARED_USAGE']) * (
                                   blocks_per_sm) / trans_df['Shm Config Size']
...
def preprocess_util2time(df_time, df, kmap, metrics, legends, util_type):
    ...
    start = math.floor(float(df_time.iloc[0]['Start']))
        ...
        float(df_time.iloc[-1]['Start']) + float(df_time.iloc[-1]['Duration']))
    ...
    df_time['right'] = (
        np.ceil((df_time['Start'] + df_time['Duration'] - start) / prec)).values
    ...
    for name in kmap:
        ...
        if util_type == 'mem':
            ...
            comp_values[:, metrics.index('REGISTER_USAGE')] = comp_values[:,
                                                              metrics.index(
                                                                  'REGISTER_USAGE')] * 1024 \
                                                              * comp_values[:,
                                                                metrics.index(
            ...
            blocks_per_sm = comp_values[:,
                            metrics.index('OCCUPANCY')] * 32 * 32 / comp_values[
                                                                    :,
                                                                    metrics.index(
                                                                        'Block Size')]
            comp_values[:, metrics.index('Shm Config Size')] = \
                (comp_values[:,
                 metrics.index('DYNAMIC_SHARED_USAGE')] + comp_values[:,
                                                          metrics.index(
                                                              'STATIC_SHARED_USAGE')]) * \
...
def comp2time(args, df_time, df, kmap):
    ...
    # constants
    metrics = ['ALU', 'FMA', 'FP16', 'FP64', 'Load/Store', 'Tensor', 'SPU',
               'Eligible']
    legends = metrics
...
def mem2time(args, df_time, df, kmap):
    ...
    metrics = ['DRAM_UTIL_PCT', 'Block Size', 'REGISTER_USAGE',
               'Shm Config Size',
    ...
    print(
        'invoks in time: {}'.format(df_time.dropna(subset=['Grid X']).shape[0]))
...
def parse_args():
    ...
    parser.add_argument('--outdir', required=False,
                        help='Output directory of graphs. '
                             'Default is the same as input dir.')

    parser.add_argument('--name', required=True,
                        help='Title of the generated graph.')

    parser.add_argument('--drop_threshold', type=float, default=0,
                        help='Drop kernels that have a '
                             'weight less than this threshold.')
...
def main():
    ...
    if 'sheet' in args.types:
        if 'comp' not in args.types or 'mem' not in args.types:
