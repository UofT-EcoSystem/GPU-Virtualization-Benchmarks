...
UINT32 baseMapGw::initSharedMemory()
{
    return 0;
}

/// [private] method accumulateJcounts
void baseMapGw::accumulateJcounts()
{
...
void baseMapGw::loadJforQ( UINT32 iQ, UINT32 nQ, UINT32 nJ )
{
...
void baseMapGw::loadJforSeedInterval()
{
...
#if TODO_CHOP_WHEN_DEBUGGED
        CDPrint( cdpCD0, "%s: calling loadJforQ (inner-loop iteration %u: nJ=%u)...", __FUNCTION__, nIterations, nJ );
#endif
        loadJforQ( iQ, nQ, nJ );
...
void baseMapGw::filterJforSeedInterval( UINT32 cbSharedPerBlock )
{
    if( m_pdbg->Dc.n )
    {
    ...
    else
    {
        ...
        CDPrint( cdpCD0, "[%d] %s::%s: DBj.D: 0x%016llx qid=0x%08x", 
                            m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__, TRACE_SQID, qidxxx );
    }
#endif
    ...
    /* CUDA global memory layout at this point (see resetGlobalMemory_LJSI):
        ...
        low:    Qw      Qwarps
                ...
                Dx      candidates for gapped alignment

        high:   Du      previously-evaluated D values
    */
...
void baseMapGw::mapJforSeedInterval()
{
#if TODO_CHOP_WHEN_DEBUGGED
    ...
    if( !isSorted )
    {
        ...
        for( UINT32 n=1; n<m_pdbg->Dx.n; ++n )
        {
            if( Dxxx.p[n] < Dxxx.p[n-1] )
                CDPrint( cdpCD0, "%s: D[%u]=0x%016llx < D[%u]=0x%016llx", __FUNCTION__, n, Dxxx.p[n], n-1, Dxxx.p[n-1] ); 
        }
        DebugBreak();       
    }
#endif
    ...
    else
    {
        ...
        for( UINT32 n=0; n<m_pdbg->Dx.n; ++n )
        {
            ...
            CDPrint( cdpCD0, "[%d] %s::%s: Dx: %4u 0x%016llx qid=0x%08x subId=%d J=%d Jf=%d Vmax=%d sqId=0x%016llx",
                        m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
...
#if TODO_CHOP_WHEN_DEBUGGED
    thrust::device_ptr<UINT64> tpDx2( m_pdbg->Dx.p );
    isSorted = thrust::is_sorted( epCGA, tpDx2, tpDx2+m_pdbg->Dx.n, TSX::isLessD() );
    if( !isSorted ) DebugBreak();
#endif
...
#if TODO_CHOP_WHEN_DEBUGGED
    CDPrint( cdpCD0, "[%d] %s::%s: Dx.n=%u", m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__, m_pdbg->Dx.n );
#endif
...
#if TODO_CHOP_WHEN_DEBUGGED
    thrust::device_ptr<UINT64> tpDx4( m_pdbg->Dx.p );
    ...
    */
    ...
    else
    {
        ...
        for( UINT32 n=0; n<m_pdbg->Dx.n; ++n )
        {
            ...
            CDPrint( cdpCD0, "[%d] %s::%s: Dm: %4u 0x%016llx qid=0x%08x subId=%d J=%d Jf=%d sqId=0x%016llx",
                        m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                        n, D, qid, subId, J, Jf, sqId );
        }
    }
    ...
    WinGlobalPtr<UINT64> Dmyyy( m_pdbg->Dm.n, false );
    m_pdbg->Dm.CopyToHost( Dmyyy.p, Dmyyy.Count );
    for( UINT32 n=0; n<m_pdbg->Dm.n; ++n )
    {
        ...
        CDPrint( cdpCD0, "[%d] %s::%s: Dm 2: %4u 0x%016llx qid=0x%08x subId=%d J=%d Jf=%d",
                            m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                            n, D, qid, subId, J, Jf );
    }
#endif
    ...
    */
    ...
    if( true )
    {
        CDPrint( cdpCD0, "%s: looking for garbagio...", __FUNCTION__ );
        WinGlobalPtr<UINT64> Dmx( m_pdbg->Dm.n, false );
        m_pdbg->Dm.CopyToHost( Dmx.p, Dmx.Count );
        for( UINT32 n=0; n<m_pdbg->Dm.n; ++n )
        {
            UINT64 D = Dmx.p[n];
    ...
    // translate back to D format; set the "candidate" flag
    tuXlatToD xD( m_pqb, m_pdbg->Dm.p, m_pdbg->Dm.n, m_pdbg->Dm.p, AriocDS::Df::flagCandidate, AriocDS::D::flagCandidate );
    xD.Start();
    xD.Wait();
    

#if TRACE_SQID
    // find the D values for the specified sqId
    CDPrint( cdpCD0, "%s: looking for concordantly-mapped Dm values for 0x%016llx...", __FUNCTION__, TRACE_SQID );
    WinGlobalPtr<UINT64> Dmmmm( m_pdbg->Dm.n, false );
    m_pdbg->Dm.CopyToHost( Dmmmm.p, Dmmmm.Count );
    for( UINT32 n=0; n<m_pdbg->Dm.n; ++n )
    {
        UINT64 D = Dmmmm.p[n];
        UINT32 qid = AriocDS::D::GetQid(D);
        ...
        if( (sqId | AriocDS::SqId::MaskMateId) == (TRACE_SQID | AriocDS::SqId::MaskMateId) )
        {
            ...
            CDPrint( cdpCD0, "[%d] %s::%s: %3d: D=0x%016llx sqId=0x%016llx qid=0x%08x subId=%d J=%d Jf=%d before baseCountC",
                                m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                                n, D, sqId, qid, subId, J, Jf );
        }
    }

    // what's the concordant-mapping status for the sqId?
    WinGlobalPtr<Qwarp> Qwmmm( m_pqb->DB.Qw.n, false );
    m_pqb->DB.Qw.CopyToHost( Qwmmm.p, Qwmmm.Count );
    for( UINT32 iw=0; iw<m_pqb->DB.Qw.n; ++iw )
    {
        Qwarp* pQw = Qwmmm.p + iw;
        ...
        for( INT16 iq=0; iq<pQw->nQ; ++iq )
        {
            ...
            if( (sqId | AriocDS::SqId::MaskMateId) == (TRACE_SQID | AriocDS::SqId::MaskMateId) )
            {
                CDPrint( cdpCD0, "[%d] %s::%s: 0x%016llx iw=%u iq=%u nAc=%d",
                                    m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                                    sqId, iw, iq, pQw->nAc[iq] );
            }
        }
    }
#endif
    ...
    baseCountC bCC( m_ptum->Key, m_pqb, m_pdbg, riDm, true );
    bCC.Start();
    bCC.Wait();
    ...
    for( UINT32 n=0; n<m_pdbg->Dm.n; ++n )
    {
        ...
        if( (sqId | AriocDS::SqId::MaskMateId) == (TRACE_SQID | AriocDS::SqId::MaskMateId) )
        {
            ...
            CDPrint( cdpCD0, "[%d] %s::%s: %3d: D=0x%016llx sqId=0x%016llx qid=0x%08x subId=%d J=%d Jf=%d after baseCountC",
                                m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                                n, D, sqId, qid, subId, J, Jf );
        }
    }
...
void baseMapGw::pruneQu()
{
    ...
    if( m_pdbg->Dc.n )
    {
        // clear the "mapped" flag and set the "candidate" flag for the Dc values
        thrust::device_ptr<UINT64> tpDc( m_pdbg->Dc.p );
        thrust::for_each( tpDc, tpDc+m_pdbg->Dc.n, TSX::initializeDflags(AriocDS::D::flagCandidate) );
        ...
        thrust::device_ptr<UINT64> tpEolDc = thrust::remove_if( epCGA, tpDc, tpDc+m_pdbg->Dc.n, TSX::isNotCandidateDvalue() );
        m_pdbg->Dc.n = static_cast<UINT32>(tpEolDc.get() - tpDc.get());
...
#if TODO_CHOP_WHEN_DEBUGGED
    CDPrint( cdpCD0, "[%d] %s::%s: m_isi=%d Dc %u/%u, Qu %u/%u, Ru %u/%u",
                        m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                        m_isi, m_pdbg->Dc.n, nD0, m_pdbg->Qu.n, nQ0, m_pdbg->Ru.n, nR0 );
#endif
}
#pragma endregion
