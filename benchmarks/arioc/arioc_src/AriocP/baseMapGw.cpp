...
baseMapGw::baseMapGw( const char* ptumKey, QBatch* pqb, DeviceBuffersG* pdbg, HostBuffers* phb ) : baseMapCommon( pqb, pdbg, phb )
{
    pqb->GetGpuConfig( "ThreadsPerBlock_baseMapGw", m_cudaThreadsPerBlock );
    m_ptum = AriocBase::GetTaskUnitMetrics( ptumKey, "baseMapGw" );
...
void baseMapGw::initGlobalMemory()
{
    CRVALIDATOR;
    ...
    try
    {
        ...
        */
        ...
        // allocate an empty buffer to contain previously-evaluated D values
...
void baseMapGw::initGlobalMemory_LJSI()
{
    CRVALIDATOR;
    ...
    /* CUDA global memory layout after initialization:
        ...
        low:    Qw      Qwarps
                Qi      interleaved Q sequence data
                DBgw.Dc [optional] mapped candidates with unmapped opposite mates
    ...
    */
    try
    {
        /* The memory-management strategy is:
            ...
            - At the start of each outer-loop iteration (in baseMapGw::main):
                - An empty consolidated Dj list (in the low memory-allocation region) is allocated.
            - The Dj list is filled in inner-loop iterations (in baseMapGw::loadJforSeedInterval)
...
void baseMapGw::initGlobalMemory_LJSIIteration( UINT32 nJ )
{
    CRVALIDATOR;
    ...
    try
    {
        ...
        */
        ...
        /* The memory-management strategy is:
            ...
            - At the start of each inner-loop iteration (in baseMapGw::loadJforSeedInterval):
                - Temporary buffers are allocated in the high memory-allocation region to handle the D values and
                    associated coverage for the iteration.
            - At the end of each inner-loop iteration:
                - The Dj list is consolidated.
                - The temporary buffers are freed.
        */

        /* Allocate buffers for the J list (D values) and associated coverage:
            - there is one element for each J value, plus one additional (null) value at the end of the buffer

           (Buffers cnJ, nJ, and oJ were allocated in baseLoadJs.)
        */
...
void baseMapGw::resetGlobalMemory_LJSIIteration()
{
    CRVALIDATOR;
    ...
    try
    {
        ...
        */
        ...
        // discard the current iteration's D buffer
...
void baseMapGw::resetGlobalMemory_LJSI()
{
    CRVALIDATOR;
...
void baseMapGw::resetGlobalMemory()
{
    /* CUDA global memory layout after reset:
