...
static __global__ void baseFilterQu_Kernel(       UINT32* const pQuBuffer,  // in,out: pointer to QIDs
                                            ...
                                            const INT16         AtG,        // in: threshold concordant-mapping count
                                          ...
                                          )
{
    ...
    /* Set the QID to null (all bits set) if it references a read that already has the user-configured
        minimum number of gapped mappings.
    */
    if( pQw->nAg[iq] >= AtG )
