...
static __global__ void tuAlignGs10_Kernel( const Qwarp* const __restrict__ pQwBuffer,   // in: Qwarps for the current batch
                                           ...
                                           const INT16                     AtN,         // in: threshold nongapped mapping count
                                                 UINT32* const             pQu          // out: QIDs of unmapped reads
                                         )
{
    ...
    /* If the current CUDA thread's Q sequence does not yet have the required number of mappings,
       ...
       The output buffer should already be initialized with null values that can be subsequently removed.  This kernel
        replaces nulls in the buffer with QIDs for reads that have fewer than the specified number of nongapped mappings
        and therefore require additional (gapped) mappings.
    */
    if( pQw->nAn[iq] < AtN )
        pQu[qid] = qid;
}
...
void tuAlignGs10::launchKernel( dim3& d3g, dim3& d3b, UINT32 cbSharedPerBlock )
{
    ...
    tuAlignGs10_Kernel<<< d3g, d3b, cbSharedPerBlock >>>( m_pqb->DB.Qw.p,       // in: Qwarps for the current batch
                                                          ...
                                                          m_pab->aas.ACP.AtN,   // in: required minimum number of nongapped mappings per read
                                                          m_pqb->DBgs.Qu.p      // out: QIDs of unmapped reads
                                                        );
}
#pragma endregion
