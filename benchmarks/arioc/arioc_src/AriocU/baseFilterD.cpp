...
/// <param name="pdbb">a reference to a <c>DeviceBuffersG</c> instance</param>
/// <param name="flagRi">D-value buffer identifier</param>
/// <param name="AtG">configured minimum number of gapped mappings per pair</param>
baseFilterD::baseFilterD( const char* ptumKey, QBatch* pqb, DeviceBuffersG* pdbg, RiFlags flagRi, INT16 AtG ) : m_pqb(pqb),
                                                                                                                m_pab(pqb->pab),
                                                                                                                m_pdbg(pdbg),
                                                                                                                m_pD(NULL), m_nD(0),
                                                                                                                m_AtG(AtG),
                                                                                                                m_ptum(AriocBase::GetTaskUnitMetrics(ptumKey,"baseFilterD"))
{
    ...
    CDPrint( cdpCD0, "[%d] %s::%s: m_AtG=%d", pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__, m_AtG );
#endif
...
void baseFilterD::copyKernelResults()
{
    ...
    CDPrint( cdpCD0, "[%d] %s::%s: D list is: D%c (AtG=%d)",
                        m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                        "?cxm"[m_pdbg->flagRi], m_AtG );
    ...
    CDPrint( cdpCD0, "[%d] %s::%s: D%c: %u candidates/%d mappings",
                        m_pqb->pgi->deviceId, m_ptum->Key, __FUNCTION__,
                        "?cxm"[m_pdbg->flagRi], nCandidates, m_nD );
#endif
...
/// <summary>
/// Flags reads as non-candidates for subsequent windowed gapped alignment
/// </summary>
