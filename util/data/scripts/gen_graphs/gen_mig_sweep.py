...
def main():
    ...
    df.drop(df.columns.difference(['pair_str', 'sm_clusters', 'norm_ipc']), 1,
            inplace=True)
    df['sm_clusters'] = pd.to_numeric(df['sm_clusters'])
    df['norm_ipc'] = pd.to_numeric(df['norm_ipc'])
    df.columns=["bm_suite", "sm_clusters", "norm_ipc"]

    df.sort_values(['bm_suite', 'sm_clusters'], ascending=[True, True],
            inplace=True)
    ...
    for suite, df_suite in df.groupby('suite'):
        ...
        df_suite.columns=['sm_clusters', 'norm_ipc', 'suite', df_suite.iat[0,2]]
    ...
    for i in range(suites):
        graph = sns.lineplot(x='sm_clusters', y='norm_ipc', marker='o',
